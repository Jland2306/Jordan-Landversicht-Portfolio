 ...
 * (/Portfolio/, see scripts/deployBase.mjs), so a hardcoded "/images/..."
 * string would otherwise resolve against the domain root and 404 there.
 * Vite only rewrites base automatically for asset references it can see
...
export function asset(path: string): string {
  const base = import.meta.env.BASE_URL // always has a trailing slash, e.g. "/" or "/Portfolio/"
